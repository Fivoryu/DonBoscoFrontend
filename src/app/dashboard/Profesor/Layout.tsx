...
import { SIDEBAR_SECTIONS_PROFESOR } from "../Sidebar/SidebarConfig";
...
export const ProfesorLayout: React.FC = () => {
  ...
  // Cargar datos de profesor cuando se tenga el usuario
  useEffect(() => {
    if (user?.id) {
    ...
    <>
      ...
      >
        <SuperAdminSB
          ...
          sections={SIDEBAR_SECTIONS_PROFESOR}
        />
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="bg-white shadow flex items-center justify-between px-6 py-3">
            ...
            <div className="flex-1 flex flex-col md:flex-row md:items-center md:gap-4">
              ...
              {/* Mostrar información del profesor si está disponible */}
              {profesor && (
                <span className="text-sm text-gray-500 md:ml-4">
                  {profesor.unidad && (
                    <>  Unidad: {profesor.unidad.nombre}
                      {profesor.unidad.colegio && (
                        <> | Colegio: {profesor.unidad.colegio.nombre}</>
                      )}
                    </>
                  )}
                </span>
